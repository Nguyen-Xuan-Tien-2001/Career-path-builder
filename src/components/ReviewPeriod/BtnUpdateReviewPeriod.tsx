...
import { Col, DatePicker, DatePickerProps, Input, Modal, Row, Select, message } from "antd"
...
const BtnUpdateReviewPeriod = (props: PropsDt) => {    
    ...
    const {dataSelectPath,data, callGetReviewByPathRefetch, setDataReviewTable} = props    
    const dateFormat = 'YYYY-MM-DD';
    const [dataSelectReview, setDataSelectReview] = useState<any>();
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [path, setPath] = useState<any>(data.pathname)
    const [reviewName, setReviewName] = useState<any>(data.reviewname)
    const [timestart, setTimeStart] = useState<any>(dayjs(data.timestart, dateFormat))
    ...
    const handleOk = () => {
        if (timeend - timestart >= 0) {
            ...
            setDataReviewTable(callGetReviewByPathRefetch(2));
    ...
    return (
        <>
        {contextHolder}
            <ButtonBase label='Sửa' icon={<EditOutlined />} className={'btn_edit'} onClick={HandleOpenModal}/>
